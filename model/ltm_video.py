...
class ltm_video_model(object):
    ...
    def write_text_cnn(self, memory):
        """
        Args:
            memory: (batch, n, dim_feature, 1) tensor
        Return:
            memory after write networks: (batch, n', 1, dim_channel)
        """

        # Kernel size = (h, w) = (FLAGS.write[0], dim_memory)
        # stride size = (h_small, infinite; it would be dropped)
        num_layer = len(self.write["kernel"])
        num_output_channels = self.write["channel"][0]

        # memory = (1, n_small, 1, num_output_channel)
        memory = layers.convolution2d(
            inputs=memory, 
            num_outputs=self.write["channel"][0],
            kernel_size=[self.write["kernel"][0], FLAGS.dim_memory],
            stride=[self.write["stride"][0], 9999],
            padding="VALID",
            weights_initializer=self.initializer_conv,
            biases_initializer=self.initializer,
            activation_fn=tf.nn.relu)
        memory = tf.reshape(memory, [-1, num_output_channels])
        return memory
    ...
    def build_model(self):
        ...
        with tf.variable_scope('memory'):
            ...
            for _ in range(self.num_hop): 
                if self.write_mode == True:
                    self.memory = self.write_text_cnn(tf.reshape(self.memory, shape=[1, -1, FLAGS.dim_memory, 1]))
